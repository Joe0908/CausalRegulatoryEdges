...
def summarize(evidence: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    positive = evidence.external_set.eq("targeted_E2")
    ...
    positive_edges = edges.external_set.eq("targeted_E2")
    ...
    summary = {
        ...
        "terminal_external_edges_passed": int(
            edges.loc[
                edges.external_set.eq("targeted_E2"),
...
def run(config_path: Path, e2_candidate_path: Path, output_dir: Path) -> None:
    config = load_config(config_path)
    parent = config["external_validation"]
    settings = parent["terminal_erythroid_fallback"]
    candidates = frozen_peak_sets(
        pd.read_csv(e2_candidate_path), str(parent["primary_tf"])
    )
...
def main() -> None:
    ...
    run(args.config, args.e2_candidates, args.output_dir)
