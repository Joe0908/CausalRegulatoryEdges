...
def build(
    ...
    selected_timepoints: list[str] | None = None,
) -> dict:
    ...
    elif selected_timepoints:
        metadata = metadata.loc[metadata.Timepoint.isin(selected_timepoints)].copy()
    ...
    summary = {
        "scope": (
            (
                "selected_timepoints:" + ",".join(selected_timepoints)
                if selected_timepoints
                else "all_timepoints"
            )
            if all_timepoints
            else "day14_late_erythroid"
        ),
...
def main() -> None:
    ...
    parser.add_argument(
        "--timepoints",
        nargs="+",
        help="Optional subset used with --all-timepoints, for example 'day 7' 'day 9'.",
    )
