"""External temporal validation of the Chapter-3 chromatin mechanism.
...
def frozen_peak_sets(evidence: pd.DataFrame, primary_tf: str) -> pd.DataFrame:
    """Return the positive and linked comparison peaks frozen in Chapter 3."""
    tf = evidence.loc[evidence.TF.eq(primary_tf)].copy()
    positive = tf.loc[tf.E2_peak.astype(bool)].copy()
    positive["external_set"] = "chapter3_E2"
    comparison = tf.loc[
...
def summarize_results(
    ...
    positive = combined.external_set.eq("chapter3_E2")
    ...
    summary = {
        ...
        "external_edges_passed": int(
            edge.loc[edge.external_set.eq("chapter3_E2"), "external_edge_pass"].sum()
        ),
